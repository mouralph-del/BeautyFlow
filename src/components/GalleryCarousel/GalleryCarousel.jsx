...
export function GalleryDetailsModal({ photo, relations = [], promotions = [], onClose, onSchedule, onPrevious, onNext, hasNavigation = false }) {
  const copy = resolveGalleryCopy(photo, relations);
  const available = relations.filter((relation) => relation.service?.active === true);
  const descriptionId = copy.description ? "gallery-service-description" : undefined;
  return createPortal(<Modal isOpen onClose={onClose} labelledBy="gallery-details-title" describedBy={available.length === 0 ? descriptionId : undefined} className="gallery-details-modal" overlayClassName="gallery-modal-overlay" closeOnOverlayClick>
    <button type="button" className="gallery-details-modal__close" aria-label="Fechar detalhes" onClick={onClose}><X /></button>
    {hasNavigation && <div className="gallery-details-modal__navigation" aria-label="Navegação da galeria">
      <button type="button" className="gallery-details-modal__nav gallery-details-modal__nav--previous" aria-label="Mídia anterior" onClick={onPrevious}><ChevronLeft /></button>
      <button type="button" className="gallery-details-modal__nav gallery-details-modal__nav--next" aria-label="Próxima mídia" onClick={onNext}><ChevronRight /></button>
    </div>}
    <div className="gallery-details-modal__layout">
      {photo?.type === "video" ? <video className="gallery-details-modal__video" src={photo.src} poster={photo.poster} controls autoPlay muted playsInline /> : <ImageWithFallback src={photo?.src} alt={photo?.alt || copy.title} className="gallery-details-modal__media" loading="eager" imgStyle={{ objectFit: "contain", objectPosition: "top center" }} />}
      <div className="gallery-details-modal__information">
        <h2 id="gallery-details-title" className="gallery-details-modal__title">{copy.title}</h2>
        {available.length === 0 && copy.description && <p id={descriptionId} className="gallery-details-modal__description">{copy.description}</p>}
...
export default function GalleryCarousel({ items, services = [], promotions = [] }) {
  ...
  const selectedIndex = selectedPhoto ? items.findIndex((item) => item.id === selectedPhoto.id) : -1;
  const selectAdjacent = (direction) => {
    if (selectedIndex < 0 || items.length < 2) return;
    setSelectedPhoto(items[(selectedIndex + direction + items.length) % items.length]);
  };
  ...
  return <>
    ...
    {selectedPhoto && <GalleryDetailsModal photo={selectedPhoto} relations={relations} promotions={promotions} onClose={() => setSelectedPhoto(null)} onSchedule={schedule} onPrevious={() => selectAdjacent(-1)} onNext={() => selectAdjacent(1)} hasNavigation={items.length > 1} />}
  </>;
}
