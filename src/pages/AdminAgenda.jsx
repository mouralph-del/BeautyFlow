...
import { cancelAdminAppointment, createAgendaBlock, createManualAppointment, createSpecialSchedule, finalizeAppointment, getAdminAgenda, updateAppointment } from "../services/adminAgenda";
...
function AppointmentDrawer({ appointment, onClose, onAction }) {
  ...
  return <><button className="agenda-drawer-overlay" onClick={onClose} aria-label="Fechar detalhes" /><aside ref={drawerRef} className="agenda-drawer" role="dialog" aria-modal="true" aria-labelledby="agenda-drawer-title" tabIndex={-1}><header><div><small>AGENDAMENTO</small><h2 id="agenda-drawer-title">{appointment.customer_name}</h2></div><button type="button" onClick={onClose} aria-label="Fechar"><X /></button></header><div className="agenda-drawer__body">
    ...
    <section className="agenda-history"><h3>Histórico</h3><p>Criado em {new Date(appointment.created_at).toLocaleString("pt-BR")}</p><p>Status atual: {statusLabel(appointment)}</p>{appointment.cancelled_at&&<><p>Cancelado em {new Date(appointment.cancelled_at).toLocaleString("pt-BR")} · {appointment.cancelled_by||"Origem não informada"}</p><p>Motivo: {appointment.reason||"Sem motivo informado"}</p>{appointment.cancelled_by_user_id&&<p className="agenda-history__audit">Registro administrativo: {appointment.cancelled_by_user_id}</p>}</>}</section>
  </div><footer><button onClick={() => onAction("edit")}>Editar</button><button onClick={() => onAction("reschedule")}>Remarcar</button>{!["concluido","cancelado","nao_compareceu"].includes(appointment.status) && appointment.payment_status !== "em_analise" && <button className="danger" onClick={() => onAction("cancel")}>Cancelar</button>}{appointment.payment_status === "em_analise" && <p className="agenda-action-hint">O pagamento deste agendamento está em análise. Analise o pagamento antes de cancelar o atendimento.</p>}{!["concluido","cancelado","nao_compareceu"].includes(appointment.status)&&<><button className="primary" onClick={() => onAction("complete")}>Concluir atendimento</button><button className="danger" onClick={() => onAction("no_show")}>Não compareceu</button><button onClick={onClose}>Manter pendente</button></>}<button onClick={() => onAction("customer")}>Ver cliente</button></footer></aside></>;
}
...
function AdminAgenda() {
  ...
  const handleAction = async (action) => { if (action === "customer") { window.location.assign(`/admin/clientes?email=${encodeURIComponent(selected.email || selected.phone)}`); return; } if (["edit","reschedule","complete","no_show","cancel-confirm"].includes(action)) { setActionError(""); return setModal(action); } if(action==="cancel"){setActionError("");setCancellationReason("");setModal("cancel-confirm");} };
  ...
  {modal==="cancel-confirm"&&selected&&<Modal title="Cancelar atendimento" onClose={()=>!saving&&setModal(null)}><form className="agenda-form" onSubmit={async(event)=>{event.preventDefault();const reason=cancellationReason.trim();if(!reason){setActionError("Informe o motivo do cancelamento.");return;}setSaving(true);setActionError("");try{await cancelAdminAppointment({appointmentId:selected.id,reason});setModal(null);setSelected(null);setCancellationReason("");await load()}catch(err){setActionError(err.message||"Não foi possível cancelar o atendimento.")}finally{setSaving(false)}}}><p>Informe o motivo para cancelar este atendimento. O horário será liberado e a reserva já paga não será estornada automaticamente.</p><label>Motivo do cancelamento<textarea required rows="4" value={cancellationReason} onChange={(event)=>setCancellationReason(event.target.value)} disabled={saving} /></label>{actionError&&<p className="agenda-error">{actionError}</p>}<div className="agenda-form__actions"><button type="button" className="secondary" disabled={saving} onClick={()=>setModal(null)}>Voltar</button><button type="submit" className="danger" disabled={saving||!cancellationReason.trim()}>{saving?"Cancelando...":"Confirmar cancelamento"}</button></div></form></Modal>}
